...
''' make_mapper method is used to output the mapper key value pair ( cmteid is the key, transactiondate  and transaction amt are its tied  values ) '''
''' define all input validation functions '''
...
def validate_date (date_text)  : 
	if ( is_not_empty(date_text)  and   len(date_text) == 8 and   datetime.datetime.strptime(date_text, '%m%d%Y')) : 
		return True 
	else :
		return  False 
...
class mapper_process (object) : 
		def __init__(self,stream):
			self.stream = stream 
		def make_output (self,cmteid,transactiondate,transactionamount) :
			'''print ('\n output' + cmteid,zipcode,transactionamount)'''
			sys.stdout.write("{0}|{1}|{2}\n".format(cmteid,transactiondate,transactionamount))    
		def make_mapper (self) :
			for line in self : 
				...
				if ( validate_other_id(otherid) and validate_cmteid(cmteid)   and validate_date (transactiondate) and  validate_trans_amt(transactionamount) )	:				 
					'''print (cmteid,zipcode,transactiondate,transactionamount,otherid) '''
					'''make value as list of cloumn and map it for each key ''' 
					self.make_output(cmteid, transactiondate,transactionamount) 	
					'''todolist''' 
