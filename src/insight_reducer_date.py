...
''' define class called reducer_process that has iterator to iterate the mapper output rows ( cmteid, transactiodate , transactionamt) in the stream (stdin) and yield a row using the generator yield '''
''' make_reducer method is used to  group the streamed data against the cmteid,transactiodate column values and find the total, median values for the grouped values  using the python itemgetter and groupby functionality '''
